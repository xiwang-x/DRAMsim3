...
if __name__ == "__main__":
    ...
    parser.add_argument("--cpu-type", default="random", choices=["random", "trace", "stream"],
                        help="cpu type")
    parser.add_argument("-t", "--trace-file", help="request trace file, only for trace from cpu to memory")
    parser.add_argument("-T", "--response-file", help="response trace file, only for trace from memory to CPU")
    ...
    for c in configs:
        ...
        if args.cpu_type == "response":
            cmd_str += " --response-file=%s" % args.response_file
