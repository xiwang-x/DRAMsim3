...
/*int Controller::InitPipe(){

		if( mkfifo(pipe_to_memory, 0666) != 0 ){
        printf( "Error creating named pipe at %s\n", pipe_to_memory);
        return 1;
    }
    mfd = open( pipe_to_memory, O_RDONLY );
    if( mfd == -1 ){
        printf( "Error opening named pipe for tracing at %s\n", pipe_to_memory);
        return 1;
    }*/

    /*if( mkfifo(pipe_to_cpu, 0666) != 0 ){
        printf( "Error creating named pipe at %s\n", pipe_to_cpu);
        return 1;
    }
    cfd = open( pipe_to_cpu, O_WRONLY | O_NONBLOCK  );
    if( cfd == -1 ){
        printf( "Error opening named pipe for tracing at %s\n", pipe_to_cpu);
        return 1;
    }
...
int Controller::WritePipe(uint64_t addr, uint64_t cycle){
      //int len = 0;
      sprintf( addr_send, "%016lx", addr );
      sprintf( cycle_send, "%16lu", cycle );
			char space[3] = " ";
			strcat(strcat(addr_send, space), cycle_send);
#ifdef DEBUG
			printf("DRAMSIM::Writing to CPU: %s\n", addr_send);
#endif
			//int ret = write( cfd, addr_send, 17+17+1 ) ;
			int ret = write( cfd, addr_send, 35 ) ;

#ifdef DEBUG
			printf("DRAMSIM::Writing to CPU Return Value: %d\n", ret);
#endif
			while(ret <= 0)
      	write( cfd, addr_send,35  );

			//printf("Memory writing; %s\n", addr_send);
      //write( mfd, cmd_send, 6 );
      //write( mfd, cycle_send, 17 );
...
/*
int Controller::ReadPipe(){
			int len = 0;

			if(read(mfd, addr_recv, 17) <= 0)
        	return -1;
      len = strlen(addrbuf);
      if( addrbuf[len] == '\n' ){
          addrbuf[len] = '\0';
...
*/
void Controller::EndPipe(){
      /*sprintf( addr_send, "%08lx", "ffffffff" );
      sprintf( cycle_send, "%d", 0 );
      sprintf( cmd_send, "%s", " END " );
      write( cfd, addr_send, 17 );
      write( cfd, cmd_send, 6 );
      write( cfd, cycle_send, 17 );
			*/
			/*char cmd[8] = " End ";
      sprintf( addr_send, "%016x", 0x0 );
      sprintf( cycle_send, "%d", 0 );
			strcat(strcat(addr_send, cmd), cycle_send);
      write( cfd, addr_send, 17+17+7);*/
			//close( cfd );
}
        ...
        if (clk >= it->complete_cycle) {
            if (it->is_write) {
                ...
                simple_stats_.AddValue("read_latency", clk_ - it->added_cycle);
								//std::cout<<"DRAMSIM::Enter WritePipe\n";
								//WritePipe( it->addr , clk_);
								//std::cout<<"DRAMSIM::Complete WritePipe\n";
            }
...
#ifdef DEBUG
						std::cout<<"DRAMSIM::READ RQST Enter WritePipe\n";
#endif
						//WritePipe( it->second.addr , clk_+config_.read_delay);
						WritePipe( cmd.hex_addr , clk_+config_.read_delay);
#ifdef DEBUG
					  std::cout<<"DRAMSIM::READ RQST Complete WritePipe\n";
#endif
