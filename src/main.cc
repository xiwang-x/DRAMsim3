...
int main(int argc, const char **argv) {
    ...
    args::ValueFlag<std::string> trace_file_arg(
        parser, "trace",
        "Request trace file, setting this option will ignore -s option",
        {'t', "trace"});
    args::ValueFlag<std::string> response_file_arg(
        parser, "response",
        "Response Trace file, setting this option will ignore -s option",
        {'T', "response"});
    ...
    std::string response_file = args::get(response_file_arg);
    std::string stream_type = args::get(stream_arg);

    CPU *cpu;
    if (!trace_file.empty()) {
        cpu = new TraceBasedCPU(config_file, output_dir, trace_file, response_file);
        //cpu = new TraceBasedCPU(config_file, output_dir, trace_file, cpu_pipe);
    } else {
